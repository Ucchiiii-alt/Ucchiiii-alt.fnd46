...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (imgSwitcher != null){
   ...
   const thumbNo = document.querySelector(".input_imgSwitcher");

   // 初期設定（最初のサムネイルをメイン画像にセット）
   mainImg.src = thumbBtns[0].querySelector("img").src;
   mainImg.alt = thumbBtns[0].querySelector("img").alt;
   //thumbBtns[0].classList.add("is_active"); //今選ばれているサムネイル
  
   // サムネイルを選択した時の処理
   for (const thumbBtn of thumbBtns){
    thumbBtn.addEventListener("click", switchImage);
    thumbBtn.addEventListener("focus", switchImage);
    }
   //インプットエリアに数字が入力されてEnter押したときの処理
   thumbNo.addEventListener("keydown", switchImage2);
   ...
   async function switchImage(){
    ...
    function switchImage2(e) {
     if (e.key === "Enter"){
      mainImg.src = "images/img" + thumbNo.value +".JPG";
      // console.log("押されました");
